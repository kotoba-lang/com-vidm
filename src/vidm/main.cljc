(ns vidm.main
  "Kotodama WASM entrypoint for the Vidm clean-room actor (L4) — Clojure port.
  ...
  facts (`vidm.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"vidm.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "vidm-compat" :tier tier :entities entities} 200])
